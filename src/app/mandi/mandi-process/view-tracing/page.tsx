...
export default function Page() {
    ...
    const fetchChartData = async () => {
        try {
            const res = await API.get(`mandi-process/tracing/chart?reelLotNo=${reelLotNo}`);
            if (res.success) {
                setData(res.data);
            }
            setLoading(false);
    ...
    return <div>
        ...
        <div className="align-items-center mt-16">
            <p style={{ textAlign: 'center' }} className="text-[20px] font-medium">Backward Traceability Tree from Mandi </p>
            <div>
                <TreeChart data={data} />
            </div>
        </div>
