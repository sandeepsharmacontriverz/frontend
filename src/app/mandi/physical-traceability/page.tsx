...
const PhysicalTraceabilityGinner = () => {
    ...
    const fetchPhysicalTraceabilityGinner = async () => {
        try {
            const res = await API.get(`physical-traceability/mandi?mandiId=${MandiId}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/mandi/physical-traceability/view-samples?id=${row.id}`}
