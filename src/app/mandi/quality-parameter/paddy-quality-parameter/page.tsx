...
const tracebaleTraining: any = () => {
  ...
  const getQualityParameter = async () => {
    const url = `quality-parameter?mandiId=${MandiId}&limit=${limit}&page=${page}&search=${code}&date=${from}&pagination=true`;
    try {
  ...
  const fetchExportAll = async () => {
    try {
      const res = await API.get(
        `quality-parameter/export?mandiId=${MandiId}&search=${code}&date=${from}`
      );
      if (res.success) {
        handleDownload(res.data, "RiceTraceability | Paddy Quality Parameter", ".xlsx");
      }
    } catch (error) {
      console.log(error);
    }
  ...
  const columns = [
    ...
    {
      name: (<p className="text-[13px] font-medium">Mandi Lot Number</p>),
      ...
      cell: (row: any) => (
        <>
          <button
            ...
            onClick={() =>
              router.push(
                `/mandi/quality-parameter/paddy-quality-parameter/detail-report-ginner?id=${row?.id}`
              )
            }
          >
