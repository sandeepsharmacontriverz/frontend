...
const PhysicalTraceabilityWeaver = () => {
    ...
    const fetchPhysicalTraceabilityWeaver = async () => {
        try {
            const res = await API.get(`physical-traceability/weaver?weaverId=${weaverId}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/weaver/physical-traceability/view-samples?id=${row.id}`}
