...
export default function page() {
  useTitle("Container Management System")
  ...
  const getContainerManagement = async () => {
    const res = await API.get(`container-management-system?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`)
    if (res.success) {
      setData(res.data);
      setCount(res.count)
    }
  ...
  const columns = [
    ...
    {
      name: "Container Management System Unit Name",
      selector: (row: any) => row.name,
      cell: (row: any) => (
        <Link href={`/settings/processor-registration/container-management-system/view-container-management-system?id=${row.id}`} >
  ...
  const filterData = async () => {
    try {
      ...
      const res = await API.get(`container-management-system?search=${searchFilter}&countryId=${checkedCountries}&stateId=${checkedStates}&limit=${limit}&page=${page}&pagination=true`);
      if (res.success) {
        setData(res.data)
        setCount(res.count)
        setShowFilter(false)
          ...
          <DeleteConfirmation
            ...
            onDelete={async () => {
              if (deleteItemId !== null) {
                const url = "container-management-system"
                try {
                            ...
                            <div className="search-bars">
                              <form className="form-group mb-0 search-bar-inner">
                                <input
                                  ...
                                  placeholder="Search by Container Management System Name"
                                  value={searchQuery}
                                  onChange={searchData}
                                />
