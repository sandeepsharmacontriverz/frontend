...
const PhysicalTraceabilityKnitter = () => {
    ...
    const fetchPhysicalTraceabilityKnitter = async () => {
        try {
            const res = await API.get(`physical-traceability/knitter?&knitterId=${knitterId}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/knitter/physical-traceability/view-samples?id=${row.id}`}
