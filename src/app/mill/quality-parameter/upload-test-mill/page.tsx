...
export default function Page() {
  ...
  const [millProcess, setMillProcess] = useState<any>();
  ...
  const getMillProcess = async () => {
    try {
      const res = await API.get(
        `mill-process/transaction?millId=${millId}&status=Sold`
      );
      if (res.success) {
        setMillProcess(res.data);
      }
    } catch (error) {
      console.log(error);
    }
  ...
  //         const matchingItem = millProcess.find(
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!hasErrors) {
      ...
      try {
        ...
        if (res.success) {
          setIsSelected(false);
          router.push("/mill/quality-parameter/cotton-quality-parameter");
  ...
  const columns = [
    ...
    {
      name: translations.mandiInterface.baleLotNo,
      sortable: false,
      width: "160px",
      center: true,
      cell: (row: any, index: any) => (
        <div className="py-2">
          ...
          >
            ...
            {millProcess?.map((millProcess: any) => (
              <option key={millProcess.id} value={millProcess.lot_no}>
                {millProcess.lot_no}
              </option>
            ))}
          </Form.Select>
    ...
    {
      name: translations.mandiInterface.reelLotNo,
      sortable: false,
      width: "220px",
      center: true,
      cell: (row: any, index: any) => (
              ...
              <div className="breadcrumb-inner light-bg">
                <div className="breadcrumb-left">
                  <ul className="breadcrum-list-wrap">
                    ...
                    <li>
                      <NavLink href="/mill/mill-process/add-mill-process">
                        {" "}
                        New Process
                      </NavLink>
                    </li>
