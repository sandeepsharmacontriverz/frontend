...
const PhysicalTraceabilitySpinner = () => {
    ...
    const fetchPhysicalTraceabilitySpinner = async () => {
        try {
            const res = await API.get(`physical-traceability/spinner?spinnerId=${spinnerId}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    }, [searchQuery, page, limit, spinnerId]);
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/mill/physical-traceability/view-samples?id=${row.id}`}
