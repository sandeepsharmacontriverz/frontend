...
export default function page() {
  ...
  useEffect(() => {
    const fetchFabricData = async () => {
      try {
        if (fabricId) {
          const res = await API.get(`dashboard/processor/fabric/buyer/type?buyer=Printing&fabric=${ fabricId }`);
          if (res.success) {
    ...
    const fetchFabricInventory = async () => {
      try {
        if (fabricId) {
          const res = await API.get(`dashboard/processor/fabric/inventory/buyer/type?buyer=Printing&fabric=${ fabricId }`);
          if (res.success) {
