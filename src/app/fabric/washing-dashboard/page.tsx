...
export default function page() {
  ...
  useEffect(() => {
    const fetchFabricData = async () => {
      try {
        if (fabricId) {
          const res = await API.get(`dashboard/processor/fabric/buyer/type?buyer=Washing&fabric=${ fabricId }`);
          if (res.success) {
    ...
    const fetchFabricInventory = async () => {
      try {
        if (fabricId) {
          const res = await API.get(`dashboard/processor/fabric/inventory/buyer/type?buyer=Washing&fabric=${ fabricId }`);
          if (res.success) {
  ...
  const columnList = [
    ...
    },
    {
      ...
      cell: (row: any) =>
       ( (row?.invoice_file && row?.invoice_file.length > 0  ) ||
        (row?.invoice_files &&  row?.invoice_files.length > 0)) && (
          <>
            <FaEye
              size={ 18 }
              className="text-black hover:text-blue-600 cursor-pointer mr-2"
              onClick={ () => handleToggleFilter(row?.invoice_file ? row?.invoice_file : row?.invoice_files) }
        ...
        (row?.invoice_files &&  row?.invoice_files.length > 0)) && (
          <>
            <FaEye
              size={ 18 }
              className="text-black hover:text-blue-600 cursor-pointer mr-2"
              onClick={ () => handleToggleAlertFilter(row?.invoice_file ? row?.invoice_file : row?.invoice_files) }
  ...
  const getFabricOptions = (): HighCharts.Options => {
    return {
      title: {
        text: 'Fabric Quantity and Fabric Stock Quantity',
        align: 'left'
      },
      chart: {
        spacingTop: 20,
      ...
      xAxis: {
        categories: fabricData ? fabricData.program : [],
      },
      series: [{
        type: 'column',
        name: "Total Quantity",
        data: fabricData ? fabricData.totalQty : [],
        color: '#ff3399'
      }, {
        type: 'column',
        name: "Stock Quantity",
        data: fabricData ? fabricData.stockQty : [],
        color: '#ff9900'
      }]
    };
  };
  ...
  const getFabricInventoryOptions = (): HighCharts.Options => {
    return {
      ...
      xAxis: {
        categories: fabricInventory ? fabricInventory.programs : [],
      },
      series: fabricInventory ? fabricInventory.fabrics : []
    };
  };
