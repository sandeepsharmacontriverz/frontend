...
export default function page() {
  ...
  const getData = async () => {
    try {
      ...
      if (res.success) {
        let total: number = 0;
        let data = res.data?.map((obj:any) => ({ ...obj, processor: "Washing", salesType : obj?.sales_type ? obj.sales_type : 'weaver' }));
  ...
  const handleSubmit = (event: any) => {
    ...
    if (uniqueProcessors.length > 1) {
      toasterError("The fabrics sold by processors vary for different selections. Please choose fabrics sold by the same processor.", 5000, fabricId);
      return;
    }
