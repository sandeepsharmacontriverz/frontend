...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
  ...
  const alreadyExistName = async (value: string) => {
    const res = await API.post("program/check-programs", {
      id: editId,
      programName: value
    });
  ...
  const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!valid || value.trim() === "") {
      newErrors['name'] = value.trim() === "" ? "Program name is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _";
    } else {
      ...
      if (!hasMergeErrors) {
        try {
          ...
          if (response.success) {
            ...
            toasterError(response.error.code === 'ALREADY_EXITS' ? 'Program Name already exist' : response.error.code, response?.data?.id, 3000);
            onCancel()
          }
        }
        catch (error) {
