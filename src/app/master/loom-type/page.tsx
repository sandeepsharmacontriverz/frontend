...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
  ...
  const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!valid || value.trim() === "") {
      newErrors['name'] = value.trim() === "" ? "Loom Type Name is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _";
    } else {
  ...
  const handleSubmit = async () => {
    ...
    if (!loomTypeName || !valid) {
      ...
      const url = "loom-type?status=true&"
      ...
      if (!hasMergeErrors) {
        try {
          ...
          if (response.success) {
            ...
            toasterError(response.error.code === 'ALREADY_EXITS' ? 'Loom Type name already exist' : response.error.code, response.data.id, 3000);
            onCancel()
          }
        }
        catch (error) {
