...
const AddSeedCompany = () => {
    ...
    const handleSubmit = (e: any) => {
        ...
        inputValues.map((inputValue, index) => {
            ...
            if (index === 0 && inputValue === '') {
                newError[index] = 'Seed company name is required';
                isError = true;
    ...
    const addData = async () => {
        ...
        const url = "seed-company/multiple";
        try {
            const response = await API.post(url, {
                seedCompanyNames: inputData
            })
            if (response.data.pass.length > 0) {
                ...
                toasterSuccess(`Following seed company(s) have been added successfully: ${passedName} `);
            }
            if (response.data.fail.length > 0) {
                ...
                toasterError(`Following seed company(s) are skipped as they are already exist: ${failedName} `);
            }
            setIsSubmitting(false);
            router.push('/master/seed-company');
        }
        catch (error) {
                    ...
                    <div className="breadcrumb-inner light-bg">
                        <div className="breadcrumb-left">
                            <ul className="breadcrum-list-wrap">
                                ...
                                <li>Master</li>
                                <li>
                                    <Link href="/master/seed-company">
                                        Seed Company
                                    </Link>
                                </li>
                                <li>Add Seed Company</li>
                            </ul>
                        </div>
                    </div>
                </div>
                            ...
                            {inputValues.map((value, index) => (
                                <div key={index} className="mb-3 mx-3">
                                    <input
                                        ...
                                        type="text"
                                        placeholder={
                                            index === 0 ? translations.seedCompany.seedCompanyName + "*" : translations.seedCompany.seedCompanyName
                                        }
                                        value={value}
