...
const SeedCompany = () => {
        ...
        if (data.length > 0) {
            const dataToExport = data.map((element: any, index: number) => {
                return {
                    ...
                    seed_company_name: element.name,
                    status: element.status
                }
            });
            exportToExcel(dataToExport, "Master-SeedCompany Data");
        } else {
    ...
    const getSeedCompanies = async () => {
        const url = `seed-company?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`;
        try {
    ...
    const columns = [
        ...
        {
            name: translations.seedCompany.seedCompanyName,
            ...
            <div>
                {isClient ? (
                    <div>
                                    ...
                                    <div className="table-minwidth min-w-[650px]">
                                        ...
                                        <div className="search-filter-row">
                                            ...
                                            <div className="search-filter-right">
                                                <button className="btn btn-all btn-purple" onClick={() => router.push("/master/seed-company/add-seed-company")} >{translations.common.add}</button>
                                            </div>
                                        </div>
                        ...
                        <div>
                            <Edit openPopup={editVisible} onCancel={handleCancel} editId={editId} defaultValue={editDefault} getItems={getSeedCompanies} />
                        </div>
...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
    const [seedCompanyName, setSeedCompanyName] = useState<string>("");
    const [errors, setErrors] = useState<{ name: string }>({ name: '' });

    const handleCancel = () => {
        onCancel();
        setErrors({ name: '' });
        setSeedCompanyName(defaultValue);
    };

    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const { value } = event.target;
        setSeedCompanyName(value);
        setErrors({ name: '' });
    };

    const handleSubmit = async () => {
        const regex: any = /^[\(\)_\-a-zA-Z0-9\s]+$/;
        const valid = regex.test(seedCompanyName);
        if (!seedCompanyName || !valid) {
            setErrors((prevError) => ({
                ...prevError,
                name: !seedCompanyName ? "Seed compnay name is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _",
            }));
        } else {
            const url = "seed-company";
            try {
                ...
                if (response.success) {
                    ...
                    toasterError(response.error.code === 'ALREADY_EXITS' ? 'Seed company name already exist' : response.error.code);
                    onCancel();
                }
            }
            catch (error) {
                    ...
                    <div className="bg-white border w-auto py-3 px-5 border-gray-300 shadow-lg rounded-md">
                        ...
                        <div className="py-2">
                            ...
                            <div className="flex py-3 justify-between">
                                <span className="text-sm mr-8">Seed Company Name * </span>
                                <div>
                                    <input
                                        type="text"
