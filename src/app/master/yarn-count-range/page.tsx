  ...
  const handleSubmit = async (formData: { [yarnCountName: string]: string }) => {
    try {
      ...
      if (res.success) {
        ...
        toasterError(res.error.code === 'ALREADY_EXITS' ? 'Yarn Count List already exist' : res.error.code, res.data.id, 3000);
        setShowEditPopup(false)
      }
    } catch (error) {
      console.log(error);
          ...
          onDelete={async () => {
            if (deleteItemId !== null) {
              ...
              try {
                ...
                if (response.success) {
                  toasterSuccess('Record has been deleted successfully', deleteItemId, 3000)
                  fetchYarnCount()
                } else {
                  toasterError('Failed to delete record', deleteItemId, 3000);
                }
              }
              catch (error) {
                console.log(error, "error")
...
const EditPopup: React.FC<EditPopupProps> = ({
  ...
  const alreadyExistName = async (value: string) => {
    const res = await API.post("yarncount/check-yarn-counts", {
      id: data?.id,
      yarnCountName: value
    });
  ...
  const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!valid || value.trim() === "") {
      newErrors['name'] = value.trim() === "" ? "Yarn Count is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _";
    } else {
