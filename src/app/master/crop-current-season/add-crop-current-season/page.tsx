...
const AddCropCurrentSeason = () => {
    useTitle("Add Crop Current Season");
    ...
    const handleSubmit = (e: any) => {
        ...
        inputValues.map((inputValue, index) => {
            ...
            if (index === 0 && inputValue === '') {
                newError[index] = 'Crop current season name is required';
                isError = true;
    ...
    const addData = async () => {
        ...
        const url = "crop-current-season/multiple";
        try {
            ...
            if (response.data.pass.length > 0) {
                ...
                toasterSuccess(`Following crop current season(s) have been added successfully: ${passedName} `);
            }
            if (response.data.fail.length > 0) {
                ...
                toasterError(`Following crop current season(s) are skipped as they are already exist: ${failedName} `);
            }
            setIsSubmitting(false);
            router.push('/master/crop-current-season');
        }
        catch (error) {
                    ...
                    <div className="breadcrumb-inner light-bg">
                        <div className="breadcrumb-left">
                            <ul className="breadcrum-list-wrap">
                                ...
                                <li>Master</li>
                                <li>
                                    <Link href="/master/crop-current-season">
                                        Crop Current Season
                                    </Link>
                                </li>
                                <li>Add Crop Current Season</li>
                            </ul>
                        </div>
                    </div>
                </div>
                            ...
                            {inputValues.map((value, index) => (
                                <div key={index} className="mb-3 mx-3">
                                    <input
                                        ...
                                        type="text"
                                        placeholder={
                                            index === 0 ? translations.cropCurrentSeason.index + "*" : translations.cropCurrentSeason.index
                                        }
                                        value={value}
...
export default AddCropCurrentSeason;