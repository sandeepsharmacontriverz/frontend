...
const CropCurrentSeason = () => {
    useTitle("Crop Current Season");
        ...
        if (data.length > 0) {
            const dataToExport = data.map((element: any, index: number) => {
                return {
                    ...
                    crop_current_season_name: element.crop_name
                }
            });
            exportToExcel(dataToExport, "Master-CropCurrentSeason Data");
        } else {
    ...
    const getCropCurrentSeasons = async () => {
        const url = `crop-current-season?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`;
        try {
    ...
    const columns = [
        ...
        {
            name: translations.cropCurrentSeason.index,
            cell: (row: any) => row.crop_name,
            sortable: false
        },
        {
            ...
            <div>
                {isClient ? (
                    <div>
                                    ...
                                    <div className="table-minwidth min-w-[650px]">
                                        ...
                                        <div className="search-filter-row">
                                            ...
                                            <div className="search-filter-right">
                                                <button className="btn btn-all btn-purple" onClick={() => router.push("/master/crop-current-season/add-crop-current-season")} >{translations.common.add}</button>
                                            </div>
                                        </div>
                        ...
                        <div>
                            <Edit openPopup={editVisible} onCancel={handleCancel} editId={editId} defaultValue={editDefault} getItems={getCropCurrentSeasons} />
                        </div>
...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
    const [cropCurrentSeasonName, setCropCurrentSeasonName] = useState<string>("");
    const [errors, setErrors] = useState<{ name: string }>({ name: '' });

    const handleCancel = () => {
        onCancel();
        setErrors({ name: '' });
        setCropCurrentSeasonName(defaultValue);
    };

    const handleInputChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const { value } = event.target;
        setCropCurrentSeasonName(value);
        setErrors({ name: '' });
    };

    const handleSubmit = async () => {
        const regex: any = /^[\(\)_\-a-zA-Z0-9\s]+$/;
        const valid = regex.test(cropCurrentSeasonName);
        if (!cropCurrentSeasonName || !valid) {
            setErrors((prevError) => ({
                ...prevError,
                name: !cropCurrentSeasonName ? "Crop current season name is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _",
            }));
        } else {
            const url = "crop-current-season";
            try {
                const response = await API.put(url, {
                    id: editId,
                    cropName: cropCurrentSeasonName
                });
                if (response.success) {
                    toasterSuccess('Record has been updated successfully');
                    setCropCurrentSeasonName('');
                    getItems();
                    onCancel();
                }
                else {
                    toasterError(response.error.code === 'ALREADY_EXITS' ? 'Crop current season name already exist' : response.error.code);
                    onCancel();
                }
            }
            catch (error) {
                    ...
                    <div className="bg-white border w-auto py-3 px-5 border-gray-300 shadow-lg rounded-md">
                        ...
                        <div className="py-2">
                            <div className="flex py-3 justify-between">
                                <span className="text-sm mr-8">Crop Current Season Name * </span>
                                <div>
                                    <input
                                        type="text"
                                        id="crop_current_season_name"
                                        name="crop_current_season_name"
                                        value={cropCurrentSeasonName}
                                        onChange={handleInputChange}
                                        className="block w-80 py-1 px-3 text-sm border border-gray-300 bg-white rounded-md"
                                        placeholder='Crop Current Season Name'
                                    />
                                    {errors.name && (
