...
const farmProduct: React.FC = () => {
  ...
  const columns = [
    ...
    {
      name: <p className="text-[13px] font-medium">{translations?.farm.farmItemName}</p>,
      cell: (row: any) => row.farmItem.farmItem,
    },
    {
      name: <p className="text-[13px] font-medium">{translations?.farm.farmProductName}</p>,
...
const Edit = ({ openPopup, onCancel, editId, farmPrevValue, farmPrevId, defaultValue, getItems }: any) => {
  ...
  const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newErrors: any = {};
    if (value.trim() !== "" && farmId !== "") {
      const onBlurError = await alreadyExistName(value);
      newErrors['farmProductName'] = onBlurError;
    } else {
      newErrors['farmProductName'] = "";
    }
    setErrors(newErrors);
  };
      ...
      if (!hasMergeErrors) {
        if (farmId && farmProductName) {
          ...
          try {
            ...
            if (response.success) {
              toasterSuccess('Record has been updated successfully', response?.ddata?.id, 3000)
              getItems()
              onCancel()
            }
            else {
              toasterError(response.error.code === 'ALREADY_EXITS' ? 'Farm Product already exists' : response.error.code, response?.ddata?.id, 3000);
              onCancel()
            }
          }
          catch (error) {
