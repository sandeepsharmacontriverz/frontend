...
const Edit = ({ openPopup, onCancel, editId, defaultStyleMark, getItems }: any) => {
    ...
    const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (!valid || value.trim() === "") {
            newErrors['name'] = value.trim() === "" ? "Style Mark No is Required" : "Enter Only Alphabets, Digits, Space, (, ), - and _";
        } else {
            ...
            if (!hasMergeErrors) {
                try {
                    ...
                    if (response.success) {
                        ...
                        toasterError(response.error.code === 'ALREADY_EXITS' ? 'Style Mark No already exist' : response.error.code, response.data.id, 3000);
                        onCancel()
                    }
                }
                catch (error) {
