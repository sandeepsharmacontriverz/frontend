  ...
  const addData = async () => {
    ...
    try {
      ...
      if (response.data.pass.length > 0) {
        ...
        toasterSuccess(`Following Rice Type(s) have been added successfully: ${passedName} `)
      }
      if (response.data.fail.length > 0) {
        ...
        toasterError(`Following Rice Type(s) are skipped as they are already exist: ${failedName} `)
      }
      router.push('/master/rice/rice-type')
    }
    catch (error) {
      setIsSubmitting(true);
  ...
  const handleSubmit = (e: any) => {
    ...
    inputValues.map((inputValue, index) => {
      ...
      if (index === 0 && inputValue === '') {
        newError[index] = 'Rice Pack type is Required';
        isError = true;
      } else if (!valid) {
        if (inputValue != '') {
          ...
          <div className="breadcrumb-inner light-bg">
            <div className="breadcrumb-left">
              <ul className="breadcrum-list-wrap">
                ...
                <li>Rice</li>
                <li>
                  <Link href="/master/rice/rice-type">
                   Rice Pack Type
                  </Link>
                </li>
                <li>Add Rice Pack Type</li>
              </ul>
            </div>
          </div>
        </div>
              ...
              {inputValues.map((value, index) => (
                <div key={index} className="mb-3 mx-3">
                  <input
                    ...
                    type="text"
                    placeholder={
                      index === 0 ? "Rice Pack Type"+ "*" : "Rice Pack Type"
                    }
                    value={value}
