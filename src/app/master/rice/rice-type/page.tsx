...
const farmItem:any = () => {
  ...
  const getRiceType = async () => {
    const url = `rice-type?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`
    try {
  ...
  const editHandle = (row: any) => {
    setEditId(row.id)
    setEditDefault(row.riceType_name)
    setEditVisible(true)
  }
  const handleCancel = () => {
    ...
    if (data.length > 0) {
      const dataToExport = data.map((element: any, index: number) => {
        return {
          ...
          riceType_name: element.riceType_name,
          riceType_status: element.riceType_status
        }
      });
      exportToExcel(dataToExport, "Master-Rice-Pack-Type Data");
    } else {
  ...
  const changeStatus = async (row: any) => {
    const newStatus = !row.riceType_status;
    const url = "rice-type/status"
    try {
  ...
  const columns = [
    ...
    {
      name: <p className="text-[13px] font-medium">Rice Pack Type</p>,

      cell: (row: any) => row.riceType_name,
    },
    {
      name: <p className="text-[13px] font-medium">{translations?.common?.status}</p>,

      cell: (row: any) => (
        <button onClick={() => changeStatus(row)} className={row.riceType_status ? "text-green-500" : "text-red-500"}>
          {row.riceType_status ? (
        ...
        {isClient ? (
          <div>
            <div className="farm-group-box">
                  ...
                  <div className="table-minwidth min-w-[650px]">
                    ...
                    <div className="search-filter-row">
                      ...
                      <div className="search-filter-right">
                        <button className="btn btn-all btn-purple" onClick={() => router.push("/master/rice/rice-type/add-rice-type")} >{translations.common.add}</button>
                      </div>
                    </div>
              ...
              <Edit openPopup={editVisible} onCancel={handleCancel} editId={editId} defaultValue={editDefault} getItems={getRiceType} />
            </div>
...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
  const [riceType_name, setRiceTypeName] = useState<any>('');
  ...
  const handleCancel = () => {
    ...
    setRiceTypeName(defaultValue)
  };

  const handleSubmit = async () => {
    const regex: any = /^[\(\)_\-a-zA-Z0-9\s]+$/;
    const valid = regex.test(riceType_name);

    if (!riceType_name || !valid) {
      if (!riceType_name) {
        setErrors((prevError) => ({
          ...prevError,
          ...
          <div className="bg-white border w-auto py-3 px-5 border-gray-300 shadow-lg rounded-md">
            ...
            <div className="py-2">
              ...
              <div className="flex py-3 justify-between">
                <span className="text-sm mr-8">Rice Pack Type * </span>
                <div>
                  <input
                    type="text"
                    id="riceType_name"
