...
const Edit = ({ openPopup, onCancel, editId, defaultValue, getItems }: any) => {
  ...
  const onBlurCheck = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!valid || value.trim() === "") {
      newErrors['name'] = value.trim() === "" ? "Unit Type is required" : "Enter Only Alphabets, Digits, Space, (, ), - and _";
    } else {
      ...
      if (!hasMergeErrors) {
        ...
        try {
          ...
          if (response.success) {
            ...
            toasterError(response.error.code === 'ALREADY_EXITS' ? 'Unit Type already exist' : response.error.code, response?.data?.id, 3000);
            onCancel()
          }
        }
        catch (error) {
