...
const WeaverFabricSale: any = () => {
    ...
    const getReportsCount = async () => {
      try {
        const response = await API.get(
          `reports/get-weaver-sales-report?page=${page}&limit=${limit}&pagination=true`
        );
        console.log(
          "Get weaver-sales Reports Count Response : ",
          response?.count
        );
        setFullDataCount(response?.count);
      } catch (error) {
        console.log("Get weaver-sales Reports Count Error : ", error);
      }
    };
    getReportsCount();
  }, []);
      ...
      async function loadFunction() {
        ...
        try {
          const response = await API.post(url, {
            file_name: "weaver-fabric-sale-report.xlsx",
          });
          ...
          if (response.data) {
            ...
            handleDownload(
              response.data,
              "Rice Traceability - Weaver Fabric Sale Report",
              ".xlsx"
            );
  ...
  const handleExport = async () => {
    const url = `reports/export-weaver-sales-report?countryId=${checkedCountries}&brandId=${
      brandId ? brandId : checkedBrands
    }&seasonId=${checkedSeasons}&programId=${checkedPrograms}&spinnerId=${checkedWeavers}&fabricType=${checkedFabric}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
    ...
    if (fullDataCount) {
      ...
      const url = `reports/export-weaver-sales-report?countryId=${checkedCountries}&brandId=${
        ...
        exportAllCountVeryFirstTime === 1
          ...
          : checkedSeasons
      }&programId=${checkedPrograms}&spinnerId=${checkedWeavers}&fabricType=${checkedFabric}&search=${searchQuery}&page=${page}&limit=${
  ...
  const getReports = async () => {
    const url = `reports/get-weaver-sales-report?countryId=${checkedCountries}&brandId=${brandId ? brandId : checkedBrands
                      ...
                      <div className="row">
                        ...
                        <div className="col-12 col-md-6 col-lg-3 mt-2">
                          ...
                          <Multiselect
                            ...
                            onSearch={function noRefCheck() {}}
                            onSelect={(selectedList: any, selectedItem: any) =>
                              handleChange(selectedList, selectedItem, "buyers")
                            }
                            options={buyer}
                            showCheckbox
                          />
