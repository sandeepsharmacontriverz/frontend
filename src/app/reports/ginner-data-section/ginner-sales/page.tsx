...
const GinnerSales: any = () => {
  ...
  const handleExport = async () => {
    const url = `reports/export-gin-sales-report?countryId=${checkedCountries}&ginnerId=${checkedGinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${
    ...
    try {
      ...
      if (response.success) {
        handleDownload(
          response.data,
          "Rice Traceability - Ginner Sales Report",
          ".xlsx"
        );
      }
  ...
  const fetchExportAll = async () => {
      ...
      const url = `reports/export-gin-sales-report?exportType=all`;
  ...
  const getReports = async () => {
    const url = `reports/get-gin-sales-report?countryId=${checkedCountries}&ginnerId=${checkedGinners}&programId=${checkedPrograms}&seasonId=${checkedSeasons}&brandId=${
      brandId ? brandId : checkedBrands
    }&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
      const response = await API.get(url);
  ...
  const handleSingleDownload = async (id: number) => {
    try {
      ...
      if (res.success) {
        handleDownload(
          res.data,
          "Rice Traceability - Lot Test Quality report",
          ".xlsx"
        );
      }
    } catch (error) {
  ...
  const columns = [
    ...
    },
    {
      name: (
        <p className="text-[13px] font-medium">
          {translations.qualityParameter.SoldTo}
        </p>
      ),
      wrap: true,
      width: "200px",
    ...
    {
      name: (
        <p className="text-[13px] font-medium"> Total Lint Quantity (Kgs) </p>
      ),
          ...
          {row?.quality_report && (
            <>
              <FaEye
                ...
                onClick={() =>
                  router.push(
                    `/reports/ginner-data-section/ginner-sales/lot-test-quality?id=${row?.quality_report?.id}`
                  )
                }
              />
              <FaDownload
    ...
    {
      name: (
        <p className="text-[12px] font-medium">{translations.common.status}</p>
      ),
      cell: (row: any) =>
        row?.status === "Sold" ? (
