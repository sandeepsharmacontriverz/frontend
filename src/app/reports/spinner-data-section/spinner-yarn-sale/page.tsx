...
const SpinnerYarnSales: any = () => {
  ...
  const handleExport = async () => {
    const url = `reports/export-spinner-yarn-sales-report?brandId=${
    ...
    try {
      ...
      if (response.success) {
        handleDownload(
          response.data,
          "Rice Traceability - Spinner Yarn Sales Report",
          ".xlsx"
        );
      }
  ...
  const fetchExportAll = async () => {
    setIsDataLoading(true);
    const url = `reports/export-spinner-yarn-sales-report?exportType=all`;
    try {
      const response = await API.get(url);
      if (response.success) {
  ...
  const getReports = async () => {
    const url = `reports/get-spinner-yarn-sales-report?brandId=${
      brandId ? brandId : checkedBrands
  ...
  const columns = [
    ...
    },
    {
      name: (
        <p className="text-[13px] font-medium">
          {translations.common.orderRefernce}
        </p>
      ),
    ...
    },
    {
      ...
      cell: (row: any) => row.yarn_type?.length > 0 ? row.yarn_type?.map((item:any)=> item)?.join(','): '',
    },
    {
      name: (
        <p className="text-[13px] font-medium">
    ...
    {
      name: (
        <p className="text-[13px] font-medium">Yarn Net Weight Sold (KGs)</p>
      ),
        ...
        row.tc_files && (
          <>
            ...
            <FaDownload
              size={18}
              className="ml-3 text-black hover:text-blue-600 cursor-pointer"
              onClick={() => handleDownloadData(row.tc_files, "tc-files")}
            />
          </>
        ),
    },
