...
const SpinnerSummaryReport: any = () => {
  ...
  const handleExport = async () => {
    const url = `reports/export-spinner-summary-report?search=${searchQuery}&countryId=${checkedCountries}&brandId=${
      brandId ? brandId : checkedBrands
    }&seasonId=${checkedSeasons}&programId=${checkedPrograms}&spinnerId=${checkedSpinners}&page=${page}&limit=${limit}&pagination=true`;
    try {
      ...
      if (response.success) {
        handleDownload(
          response.data,
          "Cotton Connect - Spinner Summary Report",
          ".xlsx"
        );
      }
  ...
  const fetchExportAll = async () => {
    setIsDataLoading(true);
    const url = `reports/export-spinner-summary-report?exportType=all`;
    try {
      const response = await API.get(url);
      if (response.success) {
        handleReportDownload(
          response.data,
          "Rice Traceability - Spinner Summary  Report",
          ".xlsx"
        );
      }
      setIsDataLoading(false);
  ...
  const getReports = async () => {
    const url = `reports/get-spinner-summary-report?search=${searchQuery}&countryId=${checkedCountries}&brandId=${
      brandId ? brandId : checkedBrands
    }&seasonId=${checkedSeasons}&programId=${checkedPrograms}&spinnerId=${checkedSpinners}&page=${page}&limit=${limit}&pagination=true`;
    try {
      const response = await API.get(url);
  ...
  const columns = [
    ...
    {
      name: (
        <p className="text-[13px] font-medium">Lint cotton processed in MT</p>
      ),
    ...
    {
      name: <p className="text-[13px] font-medium">Total Yarn Produced MT</p>,
      cell: (row: any) => formatDecimal(row?.yarnProcuredMT),
      wrap: true,
    },
    {
      name: <p className="text-[13px] font-medium">Yarn sold in MT</p>,
