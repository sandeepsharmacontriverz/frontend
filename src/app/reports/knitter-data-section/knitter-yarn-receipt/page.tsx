...
const KnitterYarnReciept: any = () => {
    ...
    const getReportsCount = async () => {
      try {
        const response = await API.get(
          `reports/get-knitter-yarn-report?page=${page}&limit=${limit}&pagination=true`
        );
        console.log(
          "Get knitter-yarn-report Reports Count Response : ",
          response?.count
        );
        setFullDataCount(response?.count);
      } catch (error) {
        console.log("Get knitter-yarn-report Reports Count Error : ", error);
      }
    };
    getReportsCount();
  }, []);
      ...
      async function loadFunction() {
        ...
        try {
          const response = await API.post(url, {
            file_name: "knitter-yarn-receipt.xlsx",
          });
          if (response.data) {
            setIsAPILoading(null);
            handleDownload(
              response.data,
              "Rice Traceability - Knitter Yarn Receipt Report",
              ".xlsx"
            );
  ...
  const handleExport = async () => {
    const url = `reports/export-knitter-yarn-report?countryId=${checkedCountries}&brandId=${
      brandId ? brandId : checkedBrands
    }&seasonId=${checkedSeasons}&programId=${checkedPrograms}&knitterId=${checkedKnitters}&spinnerId=${checkedSpinners}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
    ...
    if (fullDataCount) {
      ...
      const url = `reports/export-knitter-yarn-report?countryId=${checkedCountries}&brandId=${
        ...
        exportAllCountVeryFirstTime === 1
          ...
          : checkedSeasons
      }&programId=${checkedPrograms}&knitterId=${checkedKnitters}&spinnerId=${checkedSpinners}&search=${searchQuery}&page=${page}&limit=${
  ...
  const getReports = async () => {
    const url = `reports/get-knitter-yarn-report?countryId=${checkedCountries}&brandId=${
      brandId ? brandId : checkedBrands
    }&seasonId=${checkedSeasons}&programId=${checkedPrograms}&knitterId=${checkedKnitters}&spinnerId=${checkedSpinners}&search=${searchQuery}&page=${page}&limit=${limit}&pagination=true`;
    try {
      const response = await API.get(url);
  ...
  const columns = [
          ...
          {row?.quality_report && (
            <>
              <FaEye
                ...
                onClick={() =>
                  router.push(
                    `/reports/knitter-data-section/knitter-yarn-receipt/lot-test-quality?id=${row?.quality_report?.id}`
                  )
                }
              />
    ...
    },
    {
      ...
      selector: (row: any) => (
        <Link
          href={`/knitter/process/view-tracing?reel_lot_no=${row?.reel_lot_no}`}
