...
const PhysicalTraceabilityGarmentReport = () => {
    useTitle("Physical Tracebility Details - Garment");
    ...
    const fetchPhysicalTraceabilityGarmentReport = async () => {
        try {
            const res = await API.get(`physical-traceability/garment?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    useEffect(() => {
        fetchPhysicalTraceabilityGarmentReport();
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/reports/physical-traceability/garment-report/view-samples?id=${row.id}`}
...
export default PhysicalTraceabilityGarmentReport;