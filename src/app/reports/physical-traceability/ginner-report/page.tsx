...
const PhysicalTraceabilityGinnerReport = () => {
    useTitle("Physical Tracebility Details - Ginner");
    ...
    const fetchPhysicalTraceabilityGinnerReport = async () => {
        try {
            const res = await API.get(`physical-traceability/ginner?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    useEffect(() => {
        fetchPhysicalTraceabilityGinnerReport();
    ...
    const columns = [
        ...
        {
            name: (<p className="text-[13px] font-medium">CottonConnect Executive name</p>),
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/reports/physical-traceability/ginner-report/view-samples?id=${row.id}`}
...
export default PhysicalTraceabilityGinnerReport;