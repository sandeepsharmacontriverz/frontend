...
const PhysicalTraceabilitySpinnerReport = () => {
    useTitle("Physical Tracebility Details - Spinner");
    ...
    const fetchPhysicalTraceabilitySpinnerReport = async () => {
        try {
            const res = await API.get(`physical-traceability/spinner?limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
    ...
    useEffect(() => {
        fetchPhysicalTraceabilitySpinnerReport();
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/reports/physical-traceability/spinner-report/view-samples?id=${row.id}`}
...
export default PhysicalTraceabilitySpinnerReport;