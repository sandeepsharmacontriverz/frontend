...
const AddSpinnerResult = () => {
    useTitle("Add Spinner Result Details");
    ...
    const [physicalTraceabilitySpinner, setPhysicalTraceabilitySpinner] = useState<any>({});
    ...
    const fetchPhysicalTraceabilitySpinner = async () => {
        try {
            const res = await API.get(`physical-traceability/spinner/${id}`);
            if (res.success) {
                setPhysicalTraceabilitySpinner(res.data);
                setSampleResults(res.data.physical_traceability_data_spinner_sample.map((sample: any) => ({ id: sample.id, value: sample.sample_result, code: sample.code })));
            }
        } catch (error) {
            console.log(error);
        }
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (!isError) {
                const url = "physical-traceability/spinner/add-results";
                const mainFormData = {
                    physicalTraceabilitySpinnerId: id,
                ...
                if (mainResponse.success) {
                    ...
                    router.push("/physical-partner/spinner");
                }
            } else {
                setErrors(newErrors);
            }
    ...
    const renderSpinnerTraceabilityInformation = () => {
        return (
            <div className="details-list-group mg-t-44">
                <ul className="detail-list">
                    <li className="item">
                        <span className="label">Spinner Name:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.spinner?.name || ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Yarn REEL Lot No:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.spin_process?.reel_lot_no}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Date of sample collection:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.date_sample_collection ? dateFormatter(physicalTraceabilitySpinner.date_sample_collection) : ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Data of sample dispatch:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.data_of_sample_dispatch}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Operator name:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.operator_name}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Expected date of yarn sale:</span>
                        <span className="val">
                            {physicalTraceabilitySpinner?.expected_date_of_yarn_sale ? dateFormatter(physicalTraceabilitySpinner.expected_date_of_yarn_sale) : ""}
                        </span>
                    </li>
                </ul>
            </div>
                        ...
                        <div className="flex flex-col lg:flex-row lg:w-full gap-4 md:w-full sm:w-full ">
                            <div className='w-full'>
                                <Accordian
                                    ...
                                    content={renderSpinnerTraceabilityInformation()}
...
export default AddSpinnerResult;