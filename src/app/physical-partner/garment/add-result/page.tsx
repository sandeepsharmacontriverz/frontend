...
const AddGarmentResult = () => {
    useTitle("Add Garment Result Details");
    ...
    const [physicalTraceabilityGarment, setPhysicalTraceabilityGarment] = useState<any>({});
    ...
    const fetchPhysicalTraceabilityGarment = async () => {
        try {
            const res = await API.get(`physical-traceability/garment/${id}`);
            if (res.success) {
                setPhysicalTraceabilityGarment(res.data);
                setSampleResults(res.data.physical_traceability_data_garment_sample.map((sample: any) => ({ id: sample.id, value: sample.sample_result, code: sample.code })));
            }
        } catch (error) {
            console.log(error);
        }
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (!isError) {
                const url = "physical-traceability/garment/add-results";
                const mainFormData = {
                    physicalTraceabilityGarmentId: id,
                ...
                if (mainResponse.success) {
                    ...
                    router.push("/physical-partner/garment");
                }
            } else {
                setErrors(newErrors);
            }
    ...
    const renderGarmentTraceabilityInformation = () => {
        return (
            <div className="details-list-group mg-t-44">
                <ul className="detail-list">
                    <li className="item">
                        <span className="label">Garment Name:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.garment?.name || ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Style Mark No:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.garm_process?.style_mark_no}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Date of sample collection:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.date_sample_collection ? dateFormatter(physicalTraceabilityGarment.date_sample_collection) : ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Data of sample dispatch:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.data_of_sample_dispatch}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Operator name:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.operator_name}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Expected date of garment sale:</span>
                        <span className="val">
                            {physicalTraceabilityGarment?.expected_date_of_garment_sale ? dateFormatter(physicalTraceabilityGarment.expected_date_of_garment_sale) : ""}
                        </span>
                    </li>
                </ul>
            </div>
                    ...
                    <div className="breadcrumb-inner light-bg">
                        <div className="breadcrumb-left">
                            <ul className="breadcrum-list-wrap">
                                ...
                                <li>
                                    <Link href="/physical-partner/garment" className="active">
                                        Garment
                                    </Link>
                                </li>
                                <li>Add Result</li>
                        ...
                        <div className="flex flex-col lg:flex-row lg:w-full gap-4 md:w-full sm:w-full ">
                            <div className='w-full'>
                                <Accordian
                                    ...
                                    content={renderGarmentTraceabilityInformation()}
...
export default AddGarmentResult;