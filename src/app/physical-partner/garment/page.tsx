...
const PhysicalTraceabilityGarment = () => {
    ...
    const [garments, setGarments] = useState<Array<any>>([]);
    ...
    const fetchGarments = async () => {
        try {
            const res = await API.get("garment");
            if (res.success) {
    ...
    const fetchPhysicalTraceabilityGarment = async () => {
        try {
            const res = await API.get(`physical-traceability/garment?physicalPartnerId=${physicalPartnerId}&garmentId=${appliedFilters.garment_id}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
                                            ...
                                            <div className="row">
                                                <div className="col-12 col-md-6 col-lg-6 mt-2">
                                                    ...
                                                    <select
                                                        ...
                                                        name="garment_id"
                                                        value={appliedFilters.garment_id || ""}
    ...
    const columns = [
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/physical-partner/garment/view-samples?id=${row.id}`}
        ...
        },
        {
            ...
            cell: (row: any) => (
                row.upload_report ? (
                    <Link
                        href={`/physical-partner/garment/view-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        View Result
                    </Link>
                    ...
                    <Link
                        href={`/physical-partner/garment/add-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        Add Result
                    </Link>
