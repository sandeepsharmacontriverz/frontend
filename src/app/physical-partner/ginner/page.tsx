...
const PhysicalTraceabilityGinner = () => {
    useTitle("Ginner Details");
    ...
    const [ginners, setGinners] = useState<Array<any>>([]);
    ...
    const fetchPhysicalTraceabilityGinner = async () => {
        try {
            const res = await API.get(`physical-traceability/ginner?physicalPartnerId=${physicalPartnerId}&mandiId=${appliedFilters.ginner_id}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
                                            ...
                                            <div className="row">
                                                <div className="col-12 col-md-6 col-lg-6 mt-2">
                                                    ...
                                                    <select
                                                        ...
                                                        name="ginner_id"
                                                        value={appliedFilters.ginner_id || ""}
    ...
    const columns = [
        ...
        {
            name: (<p className="text-[13px] font-medium">Bale REEL Lot No</p>),
            selector: (row: any) => row?.gin_process?.reel_lot_no || "",
            wrap: true,
        },
        {
            name: (<p className="text-[13px] font-medium">End date of DNA marker application</p>),
            cell: (row: any) => dateFormatter(row.end_date_of_DNA_marker_application),
            wrap: true,
            width: '120px',
        },
        {
        ...
        {
            name: (<p className="text-[13px] font-medium">RiceTraceability Executive name</p>),
            selector: (row: any) => row.cotton_connect_executive_name,
            wrap: true,
        },
        {
            name: (<p className="text-[13px] font-medium">Expected date of lint sale</p>),
            cell: (row: any) => dateFormatter(row.expected_date_of_lint_sale),
            wrap: true,
            width: '120px',
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/physical-partner/ginner/view-samples?id=${row.id}`}
        ...
        },
        {
            ...
            cell: (row: any) => (
                row.upload_report ? (
                    <Link
                        href={`/physical-partner/ginner/view-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        View Result
                    </Link>
                    ...
                    <Link
                        href={`/physical-partner/ginner/add-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        Add Result
                    </Link>
...
export default PhysicalTraceabilityGinner;