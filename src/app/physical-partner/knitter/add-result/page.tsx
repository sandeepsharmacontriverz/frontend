...
const AddKnitterResult = () => {
    useTitle("Add Knitter Result Details");
    ...
    const [physicalTraceabilityKnitter, setPhysicalTraceabilityKnitter] = useState<any>({});
    ...
    const fetchPhysicalTraceabilityKnitter = async () => {
        try {
            const res = await API.get(`physical-traceability/knitter/${id}`);
            if (res.success) {
                setPhysicalTraceabilityKnitter(res.data);
                setSampleResults(res.data.physical_traceability_data_knitter_sample.map((sample: any) => ({ id: sample.id, value: sample.sample_result, code: sample.code })));
            }
        } catch (error) {
            console.log(error);
        }
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (!isError) {
                const url = "physical-traceability/knitter/add-results";
                const mainFormData = {
                    physicalTraceabilityKnitterId: id,
                ...
                if (mainResponse.success) {
                    ...
                    router.push("/physical-partner/knitter");
                }
            } else {
                setErrors(newErrors);
            }
    ...
    const renderKnitterTraceabilityInformation = () => {
        return (
            <div className="details-list-group mg-t-44">
                <ul className="detail-list">
                    <li className="item">
                        <span className="label">Knitter Name:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.knitter?.name || ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Finished Batch/Lot No:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.knit_process?.batch_lot_no}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Date of sample collection:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.date_sample_collection ? dateFormatter(physicalTraceabilityKnitter.date_sample_collection) : ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Data of sample dispatch:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.data_of_sample_dispatch}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Operator name:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.operator_name}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Expected date of fabric sale:</span>
                        <span className="val">
                            {physicalTraceabilityKnitter?.expected_date_of_fabric_sale ? dateFormatter(physicalTraceabilityKnitter.expected_date_of_fabric_sale) : ""}
                        </span>
                    </li>
                </ul>
            </div>
                    ...
                    <div className="breadcrumb-inner light-bg">
                        <div className="breadcrumb-left">
                            <ul className="breadcrum-list-wrap">
                                ...
                                <li>
                                    <Link href="/physical-partner/knitter" className="active">
                                        Knitter
                                    </Link>
                                </li>
                                <li>Add Result</li>
                        ...
                        <div className="flex flex-col lg:flex-row lg:w-full gap-4 md:w-full sm:w-full ">
                            <div className='w-full'>
                                <Accordian
                                    ...
                                    content={renderKnitterTraceabilityInformation()}
...
export default AddKnitterResult;