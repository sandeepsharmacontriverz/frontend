...
const PhysicalTraceabilityWeaver = () => {
    useTitle("Weaver Details");
    ...
    const [weavers, setWeavers] = useState<Array<any>>([]);
    ...
    const fetchPhysicalTraceabilityWeaver = async () => {
        try {
            const res = await API.get(`physical-traceability/weaver?physicalPartnerId=${physicalPartnerId}&weaverId=${appliedFilters.weaver_id}&limit=${limit}&page=${page}&search=${searchQuery}&pagination=true`);
            if (res.success) {
                setData(res.data);
                setCount(res.count);
            }
                                            ...
                                            <div className="row">
                                                <div className="col-12 col-md-6 col-lg-6 mt-2">
                                                    ...
                                                    <select
                                                        ...
                                                        name="weaver_id"
                                                        value={appliedFilters.weaver_id || ""}
                                                        onChange={(e) => handleFilterChange(e.target.name, e.target.value)}
                                                    >
                                                        <option value="">Select Weaver</option>
                                                        {weavers?.map((weaver: any) => (
                                                            <option key={weaver.id} value={weaver.id}>
                                                                {weaver.name}
                                                            </option>
                                                        ))}
                                                    </select>
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row: any) => row?.weav_process?.batch_lot_no || "",
            wrap: true,
        },
        {
        ...
        },
        {
            ...
            cell: (row: any) => (
                <Link
                    href={`/physical-partner/weaver/view-samples?id=${row.id}`}
        ...
        },
        {
            ...
            cell: (row: any) => (
                row.upload_report ? (
                    <Link
                        href={`/physical-partner/weaver/view-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        View Result
                    </Link>
                    ...
                    <Link
                        href={`/physical-partner/weaver/add-result?id=${row.id}`}
                        className="hover:text-blue-600 text-blue-500"
                    >
                        Add Result
                    </Link>
...
export default PhysicalTraceabilityWeaver;