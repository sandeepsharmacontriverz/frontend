...
const AddWeaverResult = () => {
    useTitle("Add Weaver Result Details");
    ...
    const [physicalTraceabilityWeaver, setPhysicalTraceabilityWeaver] = useState<any>({});
    ...
    const fetchPhysicalTraceabilityWeaver = async () => {
        try {
            const res = await API.get(`physical-traceability/weaver/${id}`);
            if (res.success) {
                setPhysicalTraceabilityWeaver(res.data);
                setSampleResults(res.data.physical_traceability_data_weaver_sample.map((sample: any) => ({ id: sample.id, value: sample.sample_result, code: sample.code })));
            }
        } catch (error) {
            console.log(error);
        }
    ...
    const handleSubmit = async (e: any) => {
        ...
        try {
            ...
            if (!isError) {
                const url = "physical-traceability/weaver/add-results";
                const mainFormData = {
                    physicalTraceabilityWeaverId: id,
                ...
                if (mainResponse.success) {
                    ...
                    router.push("/physical-partner/weaver");
                }
            } else {
                setErrors(newErrors);
            }
    ...
    const renderWeaverTraceabilityInformation = () => {
        return (
            <div className="details-list-group mg-t-44">
                <ul className="detail-list">
                    <li className="item">
                        <span className="label">Weaver Name:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.weaver?.name || ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Finished Batch/Lot No:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.weav_process?.batch_lot_no}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Date of sample collection:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.date_sample_collection ? dateFormatter(physicalTraceabilityWeaver.date_sample_collection) : ""}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Data of sample dispatch:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.data_of_sample_dispatch}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Operator name:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.operator_name}
                        </span>
                    </li>
                    <li className="item">
                        <span className="label">Expected date of fabric sale:</span>
                        <span className="val">
                            {physicalTraceabilityWeaver?.expected_date_of_fabric_sale ? dateFormatter(physicalTraceabilityWeaver.expected_date_of_fabric_sale) : ""}
                        </span>
                    </li>
                </ul>
            </div>
                        ...
                        <div className="flex flex-col lg:flex-row lg:w-full gap-4 md:w-full sm:w-full ">
                            <div className='w-full'>
                                <Accordian
                                    ...
                                    content={renderWeaverTraceabilityInformation()}
...
export default AddWeaverResult;