...
export default function page() {
  ...
  const [checkedCMSList, setCheckedCMSList] = useState<any>([]);
  ...
  useEffect(() => {
    if (!roleLoading && hasAccess?.processor?.includes("Third_Party_Inspection")) {
      const access = checkAccess("CMS Samples");
  ...
  const fetchCmsSamples = async () => {
    try {
      const response = await API.get(
        `third-party-sample/fetch-cms-samples?thirdPartyId=${thirdPartyId}&labId=${checkedLabList}&programId=${checkedProgramList}&cmsId=${checkedCMSList}&search=${code}&limit=${limit}&page=${page}&pagination=true`
      );
      if (response.success) {
  ...
  const fetchExport = async () => {
    try {
      ...
      if (res.success) {
        handleDownload(res.data, "CMS Samples Report", ".xlsx");
      }
    } catch (error) {
      console.log(error);
    }
            ...
            <div ref={popupRef} className="fixPopupFilters fixWidth flex h-full align-items-center w-auto z-10 fixed justify-center top-3 left-0 right-0 bottom-0 p-3 ">
              <div className="bg-white border w-auto p-4 border-gray-300 shadow-md rounded-md">
                <div className="flex justify-between align-items-center">
                  <h3 className="text-lg pb-2">CMS Samples</h3>
                  <button
                    className="text-[20px]"
  ...
  const handleChangeList = (
    ...
    if (name === "lab") {
      ...
      if (checkedCMSList.includes(itemId)) {
        setCheckedCMSList(
          checkedCMSList.filter((item: any) => item !== itemId)
        );
      } else {
        setCheckedCMSList([...checkedCMSList, itemId]);
      }
                      ...
                      <div className="col-12 col-md-6 col-lg-6 mt-2">
                        ...
                        <Multiselect
                          ...
                          selectedValues={cms?.filter((item: any) =>
                            checkedCMSList.includes(item.id)
                          )}
  ...
  const columns = [
    ...
    {
      name: (<p className="text-[13px] font-medium">Container/Lot No</p>),
    ...
    {
      name: (<p className="text-[13px] font-medium">Sample collector Details</p>),
    ...
    },
    {
      ...
      cell: (row: any) =>
          <>
                <Link
                    href={`/third-party-inspection/cms-samples/view-samples?id=${row.id}`}
                    ...
                    <div className="space-x-4">
                      ...
                      {Access?.create && (
                        <button
                          ...
                          onClick={() => router.push("/third-party-inspection/cms-samples/add-new-sample")}
                        >
                          Add New Sample
                        </button>
                      )}
