...
export default function page() {
    ...
    if (formData.lotNo) {
      ...
      if (filteredData.length > 0) {
        setFormData((prevFormData: any) => ({
          ...
          millProcessId: filteredData[0]?.id
        }));
      } else {
        setFormData((prevFormData: any) => ({
          ...prevFormData,
  ...
  const onCancel = () => {
    router.push("/third-party-inspection/sample-details");
  };
  ...
  const handleSubmit = async (event: any) => {
    ...
    if (!hasErrors) {
      try {
        ...
        setIsLoading(true);
        const response = await API.post("third-party-sample", {
          ...formData,
  ...
  const getLotData = async () => {
    try {
      const res = await API.get(`third-party-sample/get-mill-process-lot?millId=${formData.millId}`);
      if (res.success) {
        setLotData(res.data);
      }
    } catch (error) {
        ...
        <div className="breadcrumb-inner light-bg">
          <div className="breadcrumb-left">
            <ul className="breadcrum-list-wrap">
              ...
              <li>
                <NavLink href="/mill/mill-process">Sample Details</NavLink>
              </li>
        ...
        <div className="bg-white rounded-md mt-4 p-4">
            ...
            <div className="row">
              <div className="borderFix pt-2 pb-2">
                <div className="row">
                  ...
                  <div className="col-12 col-sm-6 my-2">
                    <label className="text-gray-500 text-[12px] font-medium">
                      Select Mill <span className="text-red-500">*</span>
                    </label>
                    ...
                    <Select
                      ...
                      onChange={(item: any) => {
                        handleChange("millId", item?.value);
                      }}
                    />

                    {errors?.millId !== "" && (
                      <div className="text-sm text-red-500">{errors.millId}</div>
                    )}
                  </div>
                </div>
              </div>
              ...
              <div className="col-12 col-sm-6 mt-4">
                <label className="text-gray-500 text-[12px] font-medium">
                  Choose Lot No <span className="text-red-500">*</span>
                </label>
          ...
          <div className="relative">
            <PreView
              ...
              data3={formData?.millId ? millData?.find(
                (seasonId: any) =>
                  seasonId.id === formData.millId
              )?.name
                : null}
